...
export const updateCenterWorkingHours = async (
  ...
  return await prisma.$transaction(async (tx) => {
    ...
    const trainerUpdatePromises = trainersWithWorkingHours.map(
      async (trainer) => {
        // 현재 트레이너가 가진 근무시간을 요일별로 매핑
        const trainerWorkingHoursByDay = trainer.workingHours.reduce(
          (acc, wh) => {
            acc[wh.dayOfWeek] = wh;
            return acc;
          },
        ...
        for (const dayOfWeek of allDays) {
          const oldDefaultWorkingHour = oldDefaultWorkingHours.find(
            (old) => old.dayOfWeek === dayOfWeek
          );
          const newDefaultWorkingHour = newWorkingHours.find(
            (newWh) => newWh.dayOfWeek === dayOfWeek
          );
          ...
          const wasUsingDefault =
            oldDefaultWorkingHour &&
            trainerCurrentWorkingHour &&
        ...
        // 트레이너 업데이트를 한 번에 처리
        if (
          workingHoursToDisconnect.length > 0 ||
          workingHoursToConnect.length > 0
        ) {
          await tx.trainer.update({
            ...
            data: {
              workingHours: {
                ...(workingHoursToDisconnect.length > 0 && {
                  disconnect: workingHoursToDisconnect,
                }),
                ...(workingHoursToConnect.length > 0 && {
        ...
        async (trainer) => {
          ...
          for (const dayOfWeek of allDays) {
            if (!trainerWorkingHoursByDay[dayOfWeek]) {
              // 해당 요일에 근무시간이 없는 경우
              const centerWorkingHour = center.defaultWorkingHours.find(
...
// 트레이너를 다른 센터로 이동
export const moveTrainerToCenter = async (
  trainerId: string,
  centerId: string
) => {
  return await prisma.$transaction(async (tx) => {
...
export type ICenterMachines = Awaited<ReturnType<typeof getCenterMachines>>;

// 운영 중인 피트니스 센터 목록 조회 (모든 role 접근 가능)
...
// 타입 추론 추가
export type GetFitnessCentersResult = Awaited<ReturnType<typeof getFitnessCenters>>;
export type FitnessCenterItem = GetFitnessCentersResult[0];
