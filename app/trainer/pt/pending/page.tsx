...
async function patchMutator(url: string, { arg }: { arg: any }) {
...
const TrainerPendingPtListPage = () => {
  ...
  const { data: pendingPts, error, isLoading } = useSWR<GetTrainerPendingPtsResult>(
  ...
  const { trigger: handlePtAction, isMutating } = useSWRMutation(
    ...
    patchMutator,
    {
      onSuccess: (data, key, config) => {
        ...
        // 성공 메시지 표시
        const action = config?.arg?.action;
  ...
  if (error) {
    return (
      <ErrorMessage
        ...
        action={
          <Button variant="outline" onClick={() => mutate("/api/trainer/pt/pending")}>
            다시 시도
          </Button>
        }
      />
      ...
      <div className="lg:flex lg:gap-6">
        <div className="lg:flex-1 lg:max-w-4xl">
          ...
          <div className="mb-6">
            ...
            <PageHeader
              title="승인 대기 PT 목록"
              subtitle={`총 ${pendingPts?.length || 0}개의 신청이 승인을 기다리고 있습니다`}
            />
          </div>
                      ...
                      <div className="flex items-center gap-4 flex-1">
                        ...
                        <div className="flex-1">
                          ...
                          <div className="flex items-center gap-4 text-xs text-gray-500">
                            <span>신청일: {new Date(pt.createdAt).toLocaleDateString('ko-KR')}</span>
                            <span>희망시작일: {new Date(pt.startDate).toLocaleDateString('ko-KR')}</span>
                            <span>{(pt.ptProduct.price / 10000).toFixed(0)}만원</span>
                          </div>
                        </div>
                      </div>
          ...
          <div className="bg-white rounded-lg max-w-lg w-full max-h-[90vh] overflow-y-auto">
            <div className="p-6">
              ...
              <div className="space-y-6">
                ...
                <div>
                  ...
                  {modalState.pt.member?.user.mobile && (
                    <Button
                      variant="outline"
                      size="sm"
                      onClick={() => handleCallMember(modalState.pt.member.user.mobile)}
                      className="mb-4"
                    >
                      📞 회원에게 전화하기
                    </Button>
                ...
                {/* PT 상품 정보 */}
                <div>
                  <h3 className="font-semibold text-gray-900 mb-3">PT 상품 정보</h3>
                  <div className="bg-gray-50 p-4 rounded-lg">
                    ...
                    <div className="grid grid-cols-2 gap-3 text-sm">
                      <div>
                        <span className="text-gray-600">총 횟수:</span> {modalState.pt.ptProduct.totalCount}회
                      </div>
                      <div>
                        <span className="text-gray-600">가격:</span> {(modalState.pt.ptProduct.price / 10000).toFixed(0)}만원
                      </div>
                    </div>
                  </div>
                </div>
                ...
                {modalState.pt.goals && (
                  <div>
                    <h3 className="font-semibold text-gray-900 mb-3">운동 목표</h3>
                    <div className="bg-blue-50 p-4 rounded-lg">
                      <div className="space-y-1">
                        {modalState.pt.goals.split(",").map((goal, index) => (
                          goal.trim() && (
                            <div key={index} className="flex items-center gap-2">
                              <span className="text-blue-500">✓</span>
                              <span className="text-sm text-blue-800">{goal.trim()}</span>
                            </div>
                          )
                        ))}
                      </div>
                ...
                {/* 기타 정보 */}
                <div>
                  <h3 className="font-semibold text-gray-900 mb-3">신청 정보</h3>
                  <div className="text-sm text-gray-600 space-y-1">
                    <div>신청일: {new Date(modalState.pt.createdAt).toLocaleString('ko-KR')}</div>
                    <div>희망 시작일: {new Date(modalState.pt.startDate).toLocaleDateString('ko-KR')}</div>
                  </div>
                </div>
                ...
                <div className="space-y-3 pt-4 border-t">
                  <Button
                    onClick={() => setModalState({ type: "approveWithLesson", pt: modalState.pt })}
                    disabled={isMutating}
                  ...
                  <Button
                    variant="outline"
                    onClick={() => setModalState({ type: "reject", pt: modalState.pt })}
                    disabled={isMutating}
                    className="w-full"
                  >
                    거절하기
          ...
          <div className="bg-white rounded-lg max-w-md w-full">
            <div className="p-6">
              ...
              <div className="flex items-center justify-between mb-6">
                ...
                <button
                  onClick={() => setModalState({ type: "detail", pt: modalState.pt })}
                  className="text-gray-400 hover:text-gray-600"
                  disabled={isMutating}
                >
                  <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              <div className="space-y-4">
                <div className="bg-amber-50 border border-amber-200 rounded-lg p-4">
                  <p className="text-sm text-amber-800">
                    <strong>{modalState.pt.member?.user.username || "회원"}</strong>님의 
                    <strong> {modalState.pt.ptProduct.title}</strong> 신청을 거절하시겠습니까?
                  </p>
                </div>
