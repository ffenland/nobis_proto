// app/trainer/lesson/[id]/record/LessonImageUploadModal.tsx
"use client";
...
interface LessonImageUploadModalProps {
  isOpen: boolean;
  onClose: () => void;
  lessonId: string;
...
export default function LessonImageUploadModal({
  isOpen,
  onClose,
  lessonId,
  onUploadComplete,
}: LessonImageUploadModalProps) {
  ...
  const handleUpload = useCallback(async () => {
    ...
    try {
      ...
      for (let i = 0; i < selectedFiles.length; i++) {
        ...
        setUploadProgress(
          `이미지 업로드 중... (${i + 1}/${selectedFiles.length})`
        );

        try {
          // 1. 업로드 URL 요청
          const requestBody: ImageUploadRequest = {
            entityType: "LESSON" as ImageType,
            entityId: lessonId,
            metadata: {
