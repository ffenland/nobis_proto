...
export const getMemberDashboardData = async (memberId: string): Promise<MemberDashboardData> => {
  ...
  if (confirmedPt) {
    ...
    const ptInfo: PtInfo = {
      ...
      progress: {
        completed: confirmedPt.lessons.filter(l => l.records.length > 0).length,
        total: confirmedPt.ptProduct.totalCount,
        percentage: Math.round((confirmedPt.lessons.filter(l => l.records.length > 0).length / confirmedPt.ptProduct.totalCount) * 100),
      },
    };

    // 수업 기록이 있는지 확인
    const lessonsWithRecords = confirmedPt.lessons.filter(lesson => lesson.records.length > 0);
    ...
    if (hasRecords) {
      ...
      recentLesson.records.forEach((record) => {
        ...
        record.freeSetRecords.forEach((fsr) => {
          if (fsr.freeExercise?.title && !exercises.includes(fsr.freeExercise.title)) {
        ...
        record.stretchingExerciseRecords.forEach((ser) => {
          if (ser.stretchingExercise?.title && !exercises.includes(ser.stretchingExercise.title)) {
      ...
      // 예정된 수업 조회
      const upcomingLesson = confirmedPt.lessons.find(lesson => 
        lesson.scheduledAt > currentDate
      );
      ...
      return {
        ...
        upcomingLesson: upcomingLesson ? {
          ...
          duration: Math.floor((upcomingLesson.endAt.getTime() - upcomingLesson.scheduledAt.getTime()) / (1000 * 60)), // 호환성을 위해 계산
        } : undefined,
        recentLesson: {
      ...
      const firstLesson = confirmedPt.lessons.find(lesson => 
        lesson.scheduledAt > currentDate
      );
      ...
      return {
        ...
        upcomingLesson: {
          ...
          duration: Math.floor((firstLesson.endAt.getTime() - firstLesson.scheduledAt.getTime()) / (1000 * 60)), // 호환성을 위해 계산
        },
      };
    }
  }
