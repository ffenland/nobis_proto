...
import { GetFreeExerciseDetailResult } from "@/app/services/exercise/exercise.service";
...
export default function FreeExerciseDetailPage({ params }: { params: Params }) {
  ...
  const {
    ...
    error,
    mutate,
  } = useSWR<GetFreeExerciseDetailResult>(
    exerciseId ? `/api/exercises/free/${exerciseId}` : null,
    fetcher
  );
  ...
  const handleSaveDescription = async () => {
    ...
    setIsSaving(true);
    try {
      const response = await fetch(`/api/exercises/free/${exerciseId}`, {
        method: "PATCH",
                  ...
                  <div className="flex items-center gap-4 text-sm">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-2xl font-bold text-primary">
                        {exercise._count.freeSetRecords}
                      </span>
                    </div>
                  </div>
                </div>
                <div className="badge badge-lg badge-outline">프리 운동</div>
              </div>
            </div>
          </div>
