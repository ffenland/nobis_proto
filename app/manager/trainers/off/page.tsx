...
import type { GetTrainerOffsResult } from "@/app/services/manager/manager-trainer.service";
...
export default function TrainerOffManagementPage() {
  ...
  const {
    ...
    error,
    isLoading,
  } = useSWR<GetTrainerOffsResult>("/api/manager/trainers/off", fetcher);
  ...
  const handleStateChange = async (
    ...
    try {
      const response = await fetch("/api/manager/trainers/off", {
        method: "POST",
        headers: {
      ...
      if (response.ok) {
        // SWR 데이터 재검증
        mutate("/api/manager/trainers/off");
            ...
            return (
              ...
              >
                <CardContent className="p-4">
                  ...
                  {/* PENDING 상태일 때 승인/거절 버튼 */}
                  {isExpanded && off.state === "PENDING" && (
