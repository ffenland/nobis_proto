...
const MachineDetailPage = (props: { params: IParams }) => {
  ...
  const [originalMachine, setOriginalMachine] = useState<IMachineDetail | null>(
    null
  );
  ...
  useEffect(() => {
    const fetchMachine = async () => {
      const machineData = await getMachineDetail(machineId);
      if (!machineData) return;

      setMachine(machineData);
      setOriginalMachine(machineData);
    };
    fetchMachine();
  }, [machineId]);
  ...
  useEffect(() => {
    ...
    setIsModified(JSON.stringify(machine) !== JSON.stringify(originalMachine));
  }, [machine, originalMachine]);

  // 머신 설정 수정 모달 열기
  const handleEditSetting = (setting: IMachineSetting) => {
  ...
  const handleEdit = () => {
    ...
    const changes = getChangeSummary(originalMachine, machine);
  ...
  const handleConfirmSave = async () => {
    ...
    const result = await updateMachine(machine.id, changeSummary);
  ...
  const handleDelete = () => {
    if (!machine) return;
    console.log("머신 삭제:", machine.id);
    setShowDeleteModal(false);
  };

  if (!machine) return <div>로딩중...</div>;
  ...
  return (
    <div className="container mx-auto p-2 space-y-6">
      ...
      <div className="flex justify-end gap-2">
        ...
        <button
          className={`btn btn-primary ${!isModified && "btn-disabled"}`}
          onClick={handleEdit}
        >
          수정하기
        </button>
              ...
              {selectedSetting.values.length > 0 ? (
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                  {selectedSetting.values
                    .sort((a, b) => {
                      // editingState 값이 ADDING이 맨 뒤로, NEW가 맨 앞으로
                      if (
