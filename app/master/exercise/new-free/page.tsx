...
import { GetFreeExercisesResult } from "@/app/services/exercise/exercise.service";
import {
  MediaSelector,
  ISelectedMedia,
...
export default function NewFreeExercisePage() {
  ...
  const validateExerciseName = () => {
    if (!title.trim() || !preloadedFreeExercises) {
      setIsExist(false);
      return;
    }

    const normalizedInput = removeAllSpaces(title.trim());
    const exists = preloadedFreeExercises.some((exercise) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const exerciseResponse = await fetch("/api/exercises/free", {
        method: "POST",
      ...
      alert("프리 운동이 성공적으로 생성되었습니다!");
      router.push("/master/exercise");
    } catch (error) {
  ...
  return (
    <>
      ...
      <div className="h-full container mx-auto px-4 py-8 max-w-4xl overflow-y-auto">
        <h1 className="text-2xl font-bold mb-6">새 프리 운동 등록</h1>
        ...
        <form onSubmit={handleSubmit} className="space-y-8">
          ...
          <div className="form-control">
            ...
            <textarea
              ...
              placeholder="운동에 대한 설명을 입력하세요 (선택사항)"
              rows={3}
            />
          </div>
