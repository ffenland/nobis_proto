...
import {
  IMasterFreeExerciseList,
  IMasterStretchingExerciseList,
} from "@/app/services/master/master-exercise.service";

type SortOption = "title" | "count";

const ExercisePage = () => {
  const { data, isLoading, error } = useSWR<{
    freeExercises: IMasterFreeExerciseList;
    stretchingExercises: IMasterStretchingExerciseList;
  }>("/api/master/exercises");
    ...
    <div className="h-full flex flex-col overflow-hidden">
      ...
      <div className="flex-1 flex flex-col md:flex-row gap-4 overflow-y-auto md:overflow-y-hidden min-h-0">
        ...
        <div className="flex-shrink-0 md:flex-1 flex flex-col border rounded-lg p-4 md:min-h-0">
          <div className="flex items-center md:flex-col md:items-start gap-2 justify-between mb-4">
            <h2 className="text-xl font-semibold ml-1">프리 운동</h2>
          ...
          <div className="md:overflow-y-auto md:flex-1">
            <div className="space-y-2">
              {sortedFreeExercises.map((exercise, index) => (
                <Link
                  key={index}
                  href={`/master/exercise/free/${exercise.id}`}
        ...
        <div className="flex-shrink-0 md:flex-1 flex flex-col border rounded-lg p-4 md:min-h-0">
          <div className="flex items-center md:flex-col md:items-start gap-2 justify-between mb-4">
            <h2 className="text-xl font-semibold ml-1">스트레칭</h2>
            <div className="flex items-center gap-2">
              <select
                className="select select-sm select-bordered"
              ...
              <Link
                href="/master/exercise/new-stretching"
                className="btn btn-sm btn-primary"
              >
                <Plus className="w-4 h-4" />
                새로운 운동 추가
          ...
          <div className="md:overflow-y-auto md:flex-1">
            <div className="space-y-2">
              {sortedStretchingExercises.map((exercise, index) => (
                <Link
                  key={index}
                  href={`/master/exercise/stretching/${exercise.id}`}
